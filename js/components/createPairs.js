...
export const createPairs = (app) => {
    ...
    // Создаем пустой массив dataCards который будем использовать в функции cardController, чтобы с помощью этого пустого массива сделать копию из переданного в функцию cardController массива
    let dataCards = [];
    ...
    const flipCard = () => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                // Увеличиваем значение свойства index у карточки buttonCard на 1
                buttonCard.index += 1;
    ...
    const cardController = (data) => {
        // Делаем копию переданного в функцию массива, записав его данные в массив dataCards объявленный ранее вне функции
        // Воспользуемся спрэд оператором ... чтобы поместить в новый массив элементы переданного в функцию массива, а не просто записать массив data в dataCards и получить единный массив у которого просто будет два разных названия
        dataCards = [...data];

        // Создаем в элементе buttonCard (карточка-категория с парами слов внутри) свойство index
        // Свойству задаем значение 0, значение будет изменяться, чтобы шагать по тем данным data которые нам приходят с севрера (бегать будем по свойству карточки - index)
        buttonCard.index = 0;
        let index = 0;

        // Для карточки front подставляем textContent из переданных данных БД data и индекс указываем с помошью свойства "index" заданного у карточки-категории buttonCard и как по индексу подключились к нужной паре слов указываем нулевой индекс чтобы обратиться к первому слову из этой пары
        front.textContent = data[buttonCard.index][0];

        // Для карточки back подставляем textContent из переданных данных БД data и индекс указываем с помошью свойства "index" заданного у карточки-категории buttonCard и как по индексу подключились к нужной паре слов указываем первый индекс чтобы обратиться ко второму слову из этой пары
    ...
    const unmount = () => {
        ...
        pairs.remove();
        // Снимаем обработчик события с карточки-категории, с которой до этого радотали
        buttonCard.removeEventListener('click', flipCard);
    };
